...
class Node:
    ...
    def node_count(self) -> int:
        if self.label is not None:
            ...
            return 1 + self.child_yes.subnodes_count() + self.child_no.subnodes_count()
...
for dataset in datasets:
    ...
    for run in range(1,31):
        ...
        name = "EVO-Tree"
        filename = "evo-tree_" + dataset
        serialization_filepath = "../out/evo-tree/" + filename + "_run_" + str(run) + "_tree.model"
        yaml_filepath = "../out/evo-tree/" + filename + "_run_" + str(run) + "_results.yml"
        ...
        pickle.dump(evoDt, outfile)
        outfile.close()

        # Testing
        predicted = [evoDt.predict(x) for x in test_data[:,:-1]]
        results = {
            "method": "EVO-Tree",
            "database": dataset,
            "confusion matrix": metrics.confusion_matrix(test_data[:,-1], predicted).tolist(),
            "tree": {
                "size": evoDt.root.node_count(),
                "height": evoDt.root.height(),
                "serialization": filename + "_run_" + str(run) + "_tree.model"
            },
        ...
        print("Dataset=" + dataset + ", run=" + str(run) + " is done.")
